...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Widget circleImage(int indexPath) {
    Notificationn notification = dataSource.objectAtIndexPath(indexPath);
    return CircleAvatar(
      radius: 22.0,
  ...
  Widget subTitlePostConditions(int indexPath) {
    Notificationn notification = dataSource.objectAtIndexPath(indexPath);
    return Column(
      ...
      children: <Widget>[
        notification.filter == 'like'
            ...
            : notification.filter == 'comment'
                ? Text('${notification.user.name} commented on your post')
                                                    ...
                                                    : notification.filter ==
                                                        ...
                                                        : notification.filter ==
                                                            ...
                                                            : notification
                                                                    ...
                                                                    'group_join_reject'
                                                                ? Text(
                                                                    '${notification.user.name} rejected your request to join the group')
                                                                : notification
                                                                            .filter ==
                                                                        'wire_happened'
                                                                    ? Text(
                                                                        'you have succesfully contributed ${notification.params.amount} to @${notification.params.username}')
                                                                    : notification.filter ==
                                                                            'remind'
                                                                        ? Text(
                                                                            '${notification.user.name} reposted ${notification.params.title}')
                                                                        : notification.filter == 'group_join_accept'
                                                                            ? Text(
                                                                                '${notification.user.name} has joined ${notification.entityObj != null ? notification.entityObj.name : ""}')
                                                                            : notification.filter == 'boost_accept'
                                                                                ? Text('${notification.user.name} have accepted the boost')
                                                                                : notification.filter == 'boost_reject' ? Text('${notification.user.name} have rejected the boost') : Container(),
      ],
    );
  }
        ...
        child: Slidable(
          ...
          actions: <Widget>[
            IconSlideAction(
              ...
              onTap: () => dataSource
                  .readNotifications(dataSource.notifications[indexPath].uuid),
            ),
          ],
          secondaryActions: <Widget>[
            IconSlideAction(
              ...
              icon: Icons.delete,
              onTap: () => dataSource
                  .deleteNotification(dataSource.notifications[indexPath].uuid),
            ),
          ],
          child: Container(
            ...
            child: ListTile(
              onTap: () {},
              leading: circleImage(indexPath),
              trailing: timeShow(indexPath),
              title: Text("${notification.user.name}" ?? ""),
              subtitle: subTitlePostConditions(indexPath),
            ),
          ),
        ),
      ),
